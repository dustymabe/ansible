...
PKG_MGRS = [{'path': '/usr/bin/yum', 'name': 'yum'},
            ...
            {'path': '/usr/bin/rpm-ostree', 'name': 'rpm_ostree_pkg'},
...
class PkgMgrFactCollector(BaseFactCollector):
    ...
    def _check_rh_versions(self, pkg_mgr_name, collected_facts):
        # If the we are booted into an ostree (Atomic Host, CoreOS, Silverblue)
        # then it's safe to say we need to use `rpm-ostree` to manage installation
        if os.path.exists('/run/ostree-booted') and \
           os.path.exists('/usr/bin/rpm-ostree'):
